...
class ScanStorage(QtCore.QObject, LocalLogger):
    def __init__(self, parent=None, debug_level=None, max_scans=0):
    ...
    def __init_variables(self, max_scans=10):
        self.__main_storage = []
    ...
    def insertScan(self, index=None, scan=None):
        ...
        if scan is not None and isinstance(scan, Scan) and isinstance(index, int):
            if len(self) == self.__max_scans > 0:
                self.storage.pop(0)
